...
with the reference standard over a single-sequence baseline by $+0.0172$
...
errors. Quadratic weighted kappa improves by $+0.0172$ over a single-sequence
...
$+0.0172$ (95\% CI $[+0.0064, +0.0285]$), reproducibly across training seeds and
...
pipeline-level negative control. Section~
ef{sec:results-threats} documents
...
outperform a degree-preserving random shuffle. Gradient-weighted class activation
mapping shows why: the convolutional encoder already concentrates $2.4$ times the
chance share of its attention on the annotated lesion before any prior is added.
...
validation InfoNCE $1.5162$ against a chance value of $3.4657$ --- but the
...
every seed); the anatomical identity of the relations does not. The resulting
...
preparation is complete and is reported in Section~
ef{sec:results-rq4};
