...
%
%  PROSPECTIVE THESIS-FRAMING VERSION 1.0
...
is designated \textbf{AMOG-Net} (\textbf{A}daptive \textbf{M}ulti-sequence \textbf{O}rganised-\textbf{G}raph Network). The first contribution is anatomically aligned cross-sequence
...
\laterinsert{After the confirmatory experiments are locked, add no more than one
or two sentences stating the principal empirical outcome of the thesis. This
addition may report what was found, but must not rewrite the original problem,
...
cross-entropy for Pfirrmann grading \cite{niemeyer2021a}. While recent frameworks have
begun to evaluate distance-aware ordinal metrics \cite{trinc2026beyond}, the integration of
asymmetric clinical cost with target-level relational consistency remains unexploited.
...
\laterinsert{After Chapter 4 is complete, replace this placeholder with a brief
statement of which significance claims were actually supported. For example, state
whether the main benefit was discrimination, robustness, label efficiency,
calibration or transfer. Do not infer patient benefit from model metrics.}
...
\laterinsert{At final submission, replace ``proposed contributions'' with a concise
``demonstrated contributions'' subsection listing only components supported by the
locked experiments. Components that failed should be reported as negative findings,
not retrospectively removed from the research history.}
...
Operationally, the very first phase of local deployment includes strict, protocol-compliant
de-identification and governance verification of the local clinical cohort prior to analytical processing,
ensuring complete data privacy and patient protection from project onset.
...
\laterinsert{After all confirmatory analyses are complete, insert a short table or
paragraph mapping RQ1--RQ5 to their final empirical answers. Keep detailed metrics
in the results chapter; Chapter 1 should contain only the high-level answer.}
...
\laterinsert{Once the results are complete, update the Chapter 4 description to
reflect the final retained experiment groups without changing the prospective
research questions.}
...
\laterinsert{At final submission, add a two- or three-sentence high-level summary of
the answers to RQ1--RQ5, preserving the distinction between supported contributions,
