...
formulation itself unexamined. The three gaps that follow question the
formulation: that the lumbar spine is a set of independent predictions, that
severity errors are symmetric, and that a model validated once at one
institution has been shown to generalise.
...
\subsection{Gap 2: Ordinal Structure and Clinical Cost Remain Unexploited}
...
However, two critical dimensions remain unexploited. Ordinal objectives have
scarcely been coupled with \emph{relational target consistency} across neighbouring
predictions. And the \emph{asymmetry of clinical cost} has rarely been modelled at
the loss level: grading a severe stenosis as normal is not equivalent to the
reverse, yet standard models optimise symmetric objectives. Acharya and Kansakar
are a notable exception in tracking severe-to-normal misclassifications
...
asymmetry in its evaluation metric, but the methodological contribution lies in
coupling cost-sensitive asymmetric losses with relational consistency and
calibrated uncertainty for selective prediction.

\subsection{Gap 3: Cross-Institutional Transfer Is Asserted, Not Measured}
...
The three gaps are related rather than independent. Relational structure
(Gap~1), ordinal cost-sensitivity (Gap~2) and transferability (Gap~3) all bear
on the same underlying question---whether encoding \emph{what is known about
lumbar anatomy} into the model produces grading that is more accurate, more
clinically aligned, and more portable than treating the problem as a collection
of independent image classifications. This yields the following research
questions:
...
What the field has not attempted is the common thread of the three gaps
