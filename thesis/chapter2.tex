...
learns level-specific degeneration patterns, and demonstrate that the technique
is model-agnostic, improving baselines as well as their own architecture
\cite{nguyen2026crossspine}---but the levels are still separate outputs that do
...
literature. The single exception is Baur et al., who reconstruct each disc as a
3D point cloud and grade it with a graph neural network---but the graph is
\emph{within} a disc, over its surface geometry, not \emph{across} levels or
conditions, and the reported performance was uneven (F1 0.71 at L2/3 against
0.46 overall) \cite{baur2024automated}. Nguyen et al.\ come closer in spirit,
injecting disc-level priors and showing that the technique is model-agnostic
\cite{nguyen2026crossspine}, but the levels remain independent outputs rather
than communicating nodes. No study in this review constructs a graph whose nodes
are the level--condition targets and whose edges encode adjacency, co-occurrence
...
The natural reading is that the ordinal formulations tested were inadequate
rather than that the structure is unusable, but the literature does not resolve
this. Two things are missing. Ordinal objectives have been evaluated on
\emph{disc degeneration} grading in isolation, never on the multi-target
stenosis problem where the ordinal scale interacts with severe class imbalance
(Section~\ref{sec:lr-distributions}) and with relational structure. And the
\emph{asymmetry of clinical cost} has scarcely been modelled at all: grading a
severe stenosis as normal is not equivalent to the reverse, yet almost every
reviewed system optimises a symmetric objective. Acharya and Kansakar are the
notable exception, reporting severe-to-normal misclassification as a headline
result rather than burying it in a confusion matrix
\cite{acharya2026disccentric}, and Wu et al.\ characterise their model's
error profile as specificity-oriented and draw a deployment conclusion from it
\cite{wu2026external}. The RSNA benchmark encodes the asymmetry in its
evaluation metric but nothing compels a model to optimise for it.
...
\begin{description}
  ...
  \item[RQ5] Do cost-sensitive ordinal objectives reduce clinically consequential
  errors---specifically severe cases graded as normal or mild---relative to the
  symmetric categorical objectives that dominate the literature, and does
  relational consistency across neighbouring predictions provide a usable signal
  for identifying cases that warrant human review?
\end{description}
