...
followed by the usual feed-forward transformation. Nodes lacking visual evidence ($e_{p,i}=0$) bypass message passing and LayerNorm updates entirely, hard-masking their hidden states to zero ($\mathbf{h}^{(q+1)}_i = \mathbf{0}$) to prevent LayerNorm noise amplification and feature drift across graph layers. The gate allows a node with
