...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lp{4.3cm}cp{5.0cm}@{}}
    ...
    typed graph, topology and type controls & Partial &
    Relation-specific parameterisation helps ($+0.0093$, 7/7). Neither
    anatomical component does: topology loses to nothing against an endpoint
    shuffle, and relation semantics lose against a type shuffle. The
    Transformer comparator was not built, so \enquote{graph beats sequence
    model} is untested. \\
    \addlinespace
...
of seven seeds, surviving correction at $p_{\mathrm{FDR}} = 0.009$. Two matched
controls then locate what is responsible, and neither answer is the anatomy.

Anatomically correct topology does not improve on a degree-preserving endpoint
shuffle: $+0.0020$, four seeds of seven, bounded at $1.05\%$ of baseline. That
control preserves node count, edge count, per-type counts and degree, so the two
models differ only in \emph{which} targets are connected to \emph{which}.
Anatomically correct relation labels do not improve on a type shuffle either:
$-0.0022$, four seeds of seven, with the point estimate favouring the control.
That control preserves the edge set exactly and permutes only which relation
each edge belongs to, so the two models differ only in what the three transforms
are taken to \emph{mean}.

The conclusion is therefore narrower and sharper than H3 proposed, and narrower
than the typed-versus-homogeneous comparison alone would suggest. What the graph
supplies is capacity allocated across three relation-specific transforms.
Neither the adjacency of levels, compartments and bilateral pairs that motivated
the design, nor the semantic identity assigned to each transform, is carrying
information the model uses. A graph with the right structure and scrambled
labels performs as well as the anatomical one; so does a graph with the right
labels and scrambled structure.

That is a result about relational architectures rather than about spines, and it
generalises beyond this application: a heterogeneous graph can be worth its
parameters without any of its declared semantics being load-bearing. Reporting a
typed graph as an anatomical prior, on the strength of its advantage over a
homogeneous one, would have been an overclaim that only these two controls could
detect.
