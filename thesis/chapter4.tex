...
\label{tab:steps}
\input{../data/reports/figures/table_steps}
\end{table}
...
effects are estimated to require approximately three seeds and a third
approximately seven, under the observed effect sizes and variances, and the
campaign ran seven. Where it reports nothing, that is a statement about the
effect rather than about the instrument --- subject to the caveat above, that
these requirements are themselves estimated from seven seeds.
...
one --- but it is five patients, and it is associated with the combined ordinal
and asymmetric objective rather than with any of the three anatomical mechanisms
this thesis set out to test. It should not be attributed to the cost matrix
alone: Section~\ref{sec:results-rq5-decomp} shows that the cost matrix applied
without the ordinal head makes aggregate agreement worse.
...
Anatomical topology moved in the opposite direction: from $+0.0051$ on 2/3
seeds to $+0.0020$ on 4/7. Cross-sequence self-supervision also failed to
strengthen, standing at $+0.0030$ on 4/7 after both the extension and the
initialisation correction of Section~\ref{sec:results-rq2}; its three-seed value
was $+0.0051$ on 2/3, though that figure came from the confounded configuration
...
\subsection{Nor Does the Anatomical Meaning of the Relations}
\label{sec:results-rq1-typeshuf}

\enquote{Typed edges help} admits two readings that the comparison against a
homogeneous graph cannot separate. Either the three relations carry distinct
anatomical meaning --- adjacency, cross-condition, bilateral --- which the model
exploits, or the model benefits from having three weight banks rather than one
and the assignment of edges to banks is immaterial. E5 differs from E6 in both
respects at once.

A second control separates them. It keeps the edge set exactly as the anatomical
graph and permutes only the relation labels, preserving the number of edges of
each type. The graph therefore has identical topology, an identical degree
sequence, an identical parameter count and the same three relations; the only
thing destroyed is the correspondence between a relation's parameters and its
anatomical sense.

E6 does not beat it: $-0.0022 \pm 0.0118$ on 4/7 seeds ($t = -0.49$), the point
estimate favouring the control. Ordered by mean QWK over seven seeds:

\begin{center}
\begin{tabular}{lcc}
\toprule
Configuration & Relation banks & QWK \\
\midrule
E5 homogeneous & 1 & $0.7272$ \\
E6 shuffled endpoints & 3 & $0.7344$ \\
E6 anatomical & 3 & $0.7364$ \\
E6 shuffled types & 3 & $0.7386$ \\
\bottomrule
\end{tabular}
\end{center}

The step that matters is one bank to three. Every configuration with three
relation-specific transforms performs within noise of every other, whether the
edges are grouped anatomically, grouped arbitrarily, or joined to the wrong
targets entirely.

\textbf{This narrows what RQ1 may claim.} Relation-specific parameterisation
contributes and survives correction. The anatomical content of that
parameterisation --- both which targets are connected, and what each relation is
taken to mean --- does not. The heterogeneous graph earns its place as a
capacity-allocation mechanism rather than as an encoding of spinal anatomy, and
the thesis says so rather than resting on the weaker statement that typed edges
help.

\subsection{Anatomical Topology Does Not Either}
...
The input ablation was re-run over all seven seeds when the initialisation
correction of Section~\ref{sec:results-rq2} required E4 to be rebuilt; the
figures below supersede the three-seed version reported previously.
...
Add modality dropout       & $\mathbf{-0.0758 \pm 0.0375}$ & \textbf{7/7} \\
Add ACSSL                  & $-0.0061 \pm 0.0405$ & 4/7 \\
\bottomrule
\end{tabular}
\end{center}

The router does not reduce reliance on the annotated sequence. It moves it
slightly the wrong way, on six of seven seeds. Its gate weights track the
annotation structure of the dataset without creating any causal dependence
beyond what fixed fusion already possesses.

The comparison in that table is the more informative result. Modality dropout,
which is a one-line augmentation requiring no pretext task, no correspondence
geometry and no additional training stage, reduces reliance on every seed and by
roughly twelve times the effect of the anatomically aligned self-supervision
that was designed to produce exactly this property. The extension from three
seeds to seven strengthened this contrast rather than weakening it.
...
($0.0042$), so that three seeds suffice to detect it
(Table~\ref{tab:power}).

The clinically material quantity moves in the intended direction. Recall on
Severe targets rises from $61.1\%$ at E0 to $63.1\%$ at E7, against $62.9\%$ at
E6 under categorical cross-entropy, and Section~\ref{sec:results-overgrading}
shows the improvement is not bought by predicting Severe more often. The
objective is doing what it was selected to do rather than trading distant errors
for aggregate accuracy.
...
\subsection{Sensitivity to the Cost Weight}
\label{sec:results-rq5-sweep}

The cost matrix's entries were chosen for plausibility rather than derived from
elicited clinical utilities, so the objective's weight $\lambda$ was swept over
$0$, $0.25$, $0.5$ and $1.0$, seven seeds each, with the ordinal head held
fixed.

\begin{center}
\begin{tabular}{lccc}
\toprule
Cost weight & QWK & vs E6 & Severe$\rightarrow$Normal \\
\midrule
$0.00$ & $0.7407$ & $+0.0042$ (5/7) & $\mathbf{3.52\%}$ \\
$0.25$ & $0.7370$ & $+0.0006$ (4/7) & $3.55\%$ \\
$0.50$ & $\mathbf{0.7447}$ & $\mathbf{+0.0082}$ (7/7) & $3.80\%$ \\
$1.00$ & $0.7451$ & $+0.0087$ (6/7) & $3.80\%$ \\
\bottomrule
\end{tabular}
\end{center}
...
The aggregate result is not fragile with respect to the weight, but neither is it
a dose-response. Moving from $0.5$ to $1.0$ changes QWK by $+0.0005$
($t = 0.19$), so the selected value is not a peak that a different choice would
have missed. But the $0.25$ arm sits \emph{below} the no-cost arm, and $0.5$
exceeds $0.25$ by $+0.0077$ on 6/7 seeds ($t = 3.82$). A monotone benefit in
$\lambda$ would be easier to explain than what is observed, and no mechanism
offered here accounts for the dip.

\textbf{The cost matrix does not reduce the error it was designed to penalise.}
Section~\ref{sec:method-cost} justifies the asymmetry by the cost of grading a
severe stenosis as normal, weighting that confusion four times the reverse. Over
this sweep the Severe$\rightarrow$Normal rate does not fall as $\lambda$ rises;
it is lowest with no cost matrix at all and rises slightly with weight. Adding
the cost matrix to the ordinal head changes it by $+0.29$ percentage points
($[-0.61, +1.18]$, 2/7 seeds lower).

None of those differences is distinguishable from zero at seven seeds, and the
reduction relative to E6 and to the baseline is real regardless. But it is
attributable to the ordinal head at least as much as to the cost matrix, which
is consistent with the decomposition below and inconsistent with the mechanism
the objective was justified by. The honest statement is that the combined
objective improves both aggregate agreement and the clinically asymmetric error,
and that the specific role of the cost weighting in the latter is not supported
by the sweep.

\subsection{Which of the Two Interventions Is Responsible}
\label{sec:results-rq5-decomp}

E7 changes two things at once: the head becomes ordinal, and the objective gains
the asymmetric cost matrix. Its $+0.0082$ is therefore a joint effect, and
attributing it requires running the two separately. Two further variants complete
a $2 \times 2$ design over head and objective, each over the same seven seeds.

\begin{table}[htbp]
  \centering
  \caption{Separating E7's two interventions. Every cell is the same
  architecture, differing only in the head and whether the cost matrix is
  applied.}
  \label{tab:e7decomp}
  \begin{tabular}{lcc}
    \toprule
    ...
    Cost matrix    & $0.7332$      & $\mathbf{0.7447}$ (E7) \\
    \bottomrule
  \end{tabular}
\end{table}

Neither intervention reproduces the joint effect on its own. The ordinal head
alone gives $+0.0042 \pm 0.0075$ over E6 on 5/7 seeds ($t = 1.51$). The cost
matrix alone is \emph{negative}: $-0.0032 \pm 0.0074$ on 2/7 seeds
($t = -1.14$). Added together the two main effects come to $+0.0010$, against a
measured joint effect of $+0.0082$.

The remainder is interaction, $+0.0072 \pm 0.0122$ on 6/7 seeds, some $87\%$ of
the total. A reading consistent with this is that the cost matrix supplies a
signal about the direction of error that a categorical head, whose three logits
carry no ordering, cannot use and is mildly harmed by, while the ordinal head's
monotone thresholds give that signal somewhere to act.

\textbf{That reading is not established here.} The interaction term carries
$t = 1.57$ on seven seeds; roughly twenty-three seeds would be required to
resolve it, and the individual main effects would need twenty-five and
forty-two. What the decomposition establishes is negative and still useful: the
joint effect is real and reproduces on every seed, and \emph{neither component
alone accounts for it}. Chapter~\ref{ch:discussion} previously suggested the
cost matrix was the more likely source; that suggestion is withdrawn, since the
cost matrix in isolation reduces performance.

\paragraph{Answer to RQ5.} Yes, for the combined objective, with the
attribution between its two parts unresolved. This is noted with some irony: the
objective function, explicitly disclaimed in
...
RQ & Verdict & Basis \\
\midrule
RQ1 & Partly & Relation-specific parameterisation helps: $+0.0093$,
\textbf{7/7 seeds}, $p_{\mathrm{FDR}} = 0.009$. Neither anatomical component
does. Topology does not beat a degree-preserving endpoint shuffle ($+0.0020$,
4/7); relation semantics do not beat a type shuffle ($-0.0022$, 4/7). \\
