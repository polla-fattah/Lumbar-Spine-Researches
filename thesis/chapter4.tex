...
The current set is 100 sheets over 20 studies at all five levels, 492 targets,
with 1\% truncated crops and a median cross-plane distance of $1.2$~mm --- well
inside the $3.5$--$4.8$~mm slice thickness, so the projected point lies within
the displayed slice rather than being extrapolated to it.
...
\emph{Graph configurations excluded from two probes.} The input ablation and
attribution analyses cover E0--E4 only; E5--E7 route through a different batch
layout. The routing claim is cleanest to test at E2 with the graph out of the
path, but the attribution result is therefore not measured on the final system.
