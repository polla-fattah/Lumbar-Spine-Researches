...
Two reference points fix the scale. First, the reported inter-reader agreement
for these conditions ranges from $\kappa = 0.49$ for subarticular stenosis to
$\kappa = 0.73$ for the central canal, so the disagreement between two qualified
readers spans roughly $0.24$ in kappa. The complete system's advantage over the
baseline, $+0.0177$, is about $7\%$ of that span; typed edges alone are about
$4\%$. Second, and more directly, no radiologist reading these studies would
encounter a difference of one target in seven patients as a change in the
...
\begin{figure}[htbp]
  ...
  predicted class logit, and the CORN head emits two cumulative-threshold logits
